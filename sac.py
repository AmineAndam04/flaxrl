...
class Actor(nnx.Module):
    ...
    def __call__(self, obs: jnp.ndarray, key):
        ...
        pre_tanh = mean + std * epsilon
        # squash and scale
        squashed_action = jnp.tanh(pre_tanh)
        action = squashed_action * self.action_scale + self.action_bias
        # log_prob
        log_prob = -0.5 * epsilon**2 - log_std - 0.5 * jnp.log(2.0 * jnp.pi)
...
class Temperature(nnx.Module):
    def __init__(self, initial_alpha: float, target_ent: float):
        self.log_alpha = nnx.Param(jnp.log(jnp.asarray(initial_alpha)))
...
if __name__ == "__main__":
    ...
    temperature = Temperature(initial_alpha=args.alpha, target_ent=-float(env.action_size))
    # Optimizers
    optim = getattr(optax, args.optimizer)
    actor_optimizer = optim(learning_rate=args.learning_rate_actor, eps=1e-5)
    critic_optimizer = optim(learning_rate=args.learning_rate_critic, eps=1e-5)
    temp_optimizer = optim(learning_rate=args.learning_rate_critic, eps=1e-5)
    ...
    temp_optimizer = nnx.Optimizer(temperature, temp_optimizer, wrt=nnx.Param)
    # Logging
    time_token = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    run_name = f"{args.env_name}__{args.exp_name}__{time_token}"
    log_dir = f"{args.work_dir}/SAC-{run_name}"
    # Reset the env
    ...
    def update_step(update_state: tuple, _):  # noqa: PLR0915
        ...
        # Get action
        action, _ = actor(obs=obs, key=key_act)
        # step the env
        ...
        def update_actor_and_critic(
            ...
            def actor_loss(actor):
                ...
                q2_values = critic["qnet2"](obs=batch.first.obs, action=action)
                q_values = jnp.minimum(q1_values, q2_values)
                ac_loss = alpha * log_prob - q_values
                return ac_loss.mean(), log_prob

            # update the actor
            (ac_loss, log_prob), grads = nnx.value_and_grad(actor_loss, has_aux=True)(actor)
            actor_optimizer.update(actor, grads)

            # ----- Update temperature
            def update_temp(temperature, temp_optimizer):
                # temp_loss
                def temp_loss(temperature):
                    loss = temperature() * (log_prob + temperature.target_ent)
                    return -loss.mean()

                # update temperature
                grads = nnx.grad(temp_loss)(temperature)
                temp_optimizer.update(temperature, grads)

            nnx.cond(args.autotune, update_temp, lambda x, y: None, temperature, temp_optimizer)
        ...
        key, key_sample, key_act = jax.random.split(key, 3)
    ...
    if args.eval:
        # Create eval env
        actor, _, _, _, _, _, _, rollout_state, _ = update_state
    ...
    if args.log:
        ...
        cr_losses, ac_losses, alphas, update_events, dones, episode_returns, episode_lengths = metrics
