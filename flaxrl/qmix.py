...
def train(args):  # noqa: PLR0915
    ...
    nets = nnx.Dict({"qnetwork": qnetwork, "mixer": mixer})
    target_nets = nnx.Dict({"qnetwork": target_qnetwork, "mixer": target_mixer})
    ...
    buffer = rb_fun.init(  # initialize the buffer
        TimeStep(
            ...
            state=jnp.zeros(env.state_size),
    ...
    rollout_state = RolloutState(
        obs=obs, state=state, env_state=env_state, avail_actions=avail_actions, step=0, key=key
    )
    ...
    def update_step(update_state: tuple, _):  # noqa: PLR0915
        ...
        nets, target_nets, optimizer, rollout_state, buffer = update_state

        # ------ Collect env steps ------
        obs, state, env_state, avail_actions, step, key = rollout_state
        ...
        # ------ Update the networks ------
        def update_qnetwork(nets, optimizer, key_sample):
            # Sample a batch
            batch = rb_fun.sample(buffer, key_sample).experience
            # Compute targets
            qvals_agents_next = target_nets["qnetwork"](
                obs=batch.second.obs, avail_actions=batch.second.avail_actions
            )
            qvals_agents_next = jnp.max(qvals_agents_next, axis=-1)
            q_tot_target = target_nets["mixer"](Q=qvals_agents_next, s=batch.second.state).reshape(-1)
            targets = batch.first.reward + args.gamma * (1 - batch.first.done) * q_tot_target

            def qmix_loss(nets):
                qvals = jnp.take_along_axis(
                    arr=nets["qnetwork"](obs=batch.first.obs, avail_actions=batch.first.avail_actions),
                    indices=jnp.expand_dims(batch.first.action, axis=-1),
                    axis=-1,
                ).squeeze(axis=-1)
                q_tot = nets["mixer"](Q=qvals, s=batch.first.state).reshape(-1)
                return optax.l2_loss(targets, q_tot).mean()

            # Update the q_network
            loss, grads = nnx.value_and_grad(qmix_loss)(nets)
            optimizer.update(nets, grads)
            return loss
        ...
        update_state = nets, target_nets, optimizer, rollout_state, buffer
    ...
    nets, _, _, rollout_state, _ = update_state
    qnetwork = nets["qnetwork"]
