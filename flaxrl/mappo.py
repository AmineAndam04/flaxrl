...
def train(args):
    ...
    def update_step(update_state: tuple, _):  # noqa: PLR0915
        ...
        # ------ Collect env steps ------
        def collect_rollout(actor: nnx.Module, critic: nnx.Module, rollout_state: RolloutState):
        ...
        # ------ Update the networks ------
        # ppo_epochs: run MAPPO updates for args.n_epochs epochs
        ...
        ac_batches = jax.tree.map(lambda x: jax.lax.collapse(x, 0, 3), ac_batches)
        ...
        ac_batches = jax.tree.map(lambda x: x[permutation], ac_batches)
        ac_batches = jax.tree.map(
            lambda x: x.reshape((num_batches, args.batch_size) + x.shape[1:]), ac_batches
        )
        ...
        cr_batches = jax.tree.map(
            lambda x: x.reshape((num_batches // env.num_agents, args.batch_size) + x.shape[1:]), cr_batches
        )
        # Train for n_epochs
