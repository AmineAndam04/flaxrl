...
def train(args):
    ...
    num_batches = args.num_envs * env.num_agents // args.batch_size
    ...
    def update_step(update_state: tuple, _):  # noqa: PLR0915
        ...
        def collect_rollout(
            ...
            def env_one_step(carry, x):
                # Last rollout state
                obs, env_state, avail_actions, lstm_carry, episode_start, key = carry
                # Reset hidden states
                ...
                done = jnp.broadcast_to(done, value.shape)
                ...
                reward = jnp.broadcast_to(reward, value.shape)
        ...
        def ppo_epoch(carry, batches):
            ...
            def ppo_batch(carry, batch):
                ...
                def ppo_actor_loss(
                    actor, b_obs, b_action, b_avail_actions, b_log_probs, b_adv, lstm_carry, b_episode_start
                ):
                    pi = actor.sequence(
                        carry=lstm_carry,
                        obs=b_obs,
        ...
        batches = jax.tree.map(lambda x: jax.lax.collapse(x, 1, 3), batches)
        ...
        batches = (*batches, initial_lstm_carry)
        # Train for n_epochs
