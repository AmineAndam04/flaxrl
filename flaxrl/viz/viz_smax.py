...
from envs.smax import SMAXInterface
from envs.mwrappers import VecMARLWrapper, AgentID
...
if __name__ == "__main__":
    ...
    # env
    env = SMAXInterface.make(train_args.env_name)
    ...
    jitted_avail_actions = jax.jit(env.get_avail_actions)
    obs, _, env_state = jitted_reset(reset_key)
    avail_actions = jitted_avail_actions(env_state)
    step = 0
    state_seq = []
    while True:
        key, key_step = jax.random.split(key)
        key_step = jax.random.split(key_step, 1)
        action = policy.get_action(obs, avail_actions=avail_actions)
        next_obs, _, next_env_state, reward, _, _, infos = jitted_step(key_step, env_state, action)
        done = infos["__all__"].squeeze().astype(bool)
        avail_actions = jitted_avail_actions(next_env_state)
        action = {agent: action[:, i] for i, agent in enumerate(env.unwrapped.env.agents)}
        state_seq.append(jax.tree.map(lambda x: x.squeeze(0), (key_step, env_state, action)))
    ...
    # Rendering
    viz = SMAXVisualizer(env.unwrapped.env, state_seq)
    os.makedirs(f"{args.output}/{train_args.env_name}", exist_ok=True)
    viz.animate(view=False, save_fname=f"{args.output}/{train_args.env_name}/ippo.gif")
